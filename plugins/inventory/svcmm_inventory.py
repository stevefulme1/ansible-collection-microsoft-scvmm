...
EXAMPLES = r"""
# svcmm_inventory.yml
plugin: stevefulme1.svcmm.svcmm_inventory
...
class InventoryModule(BaseInventoryPlugin):
    NAME = "stevefulme1.svcmm.svcmm_inventory"

    def verify_file(self, path):
        if super().verify_file(path):
            return path.endswith(("svcmm_inventory.yml", "svcmm_inventory.yaml"))
        return False
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        group = self.inventory.add_group("svcmm")
        ...
        try:
            ...
            for item in resp.json().get("data", []):
                ...
                if hostname:
                    self.inventory.add_host(hostname, group="svcmm")
