...
"""Unit tests for scvmm_capability_profile module documentation stub."""
...
class TestArgumentSpec:
    ...
    def test_fabric_capability_option(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        assert "fabric_capability" in opts
        assert opts["fabric_capability"]["type"] == "str"
...
class TestReturn:
    ...
    def test_return_has_profile_key(self, module_docs):
        _doc, _examples, returns = module_docs
        parsed = yaml.safe_load(returns)
        assert "profile" in parsed

    def test_return_has_changed_key(self, module_docs):
        _doc, _examples, returns = module_docs
        parsed = yaml.safe_load(returns)
