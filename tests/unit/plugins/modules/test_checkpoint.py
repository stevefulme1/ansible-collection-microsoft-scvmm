...
"""Unit tests for scvmm_vm_checkpoint module documentation stub."""
...
class TestArgumentSpec:
    def test_vm_name_option_required(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        assert "vm_name" in opts
        assert opts["vm_name"].get("required") is True

    def test_name_option(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        assert "name" in opts
    ...
    def test_state_option(self, parsed_doc):
        ...
        assert set(opts["state"]["choices"]) == {"present", "absent", "restored"}
...
class TestReturn:
    ...
    def test_return_has_checkpoint_key(self, module_docs):
        _, _, returns = module_docs
        parsed = yaml.safe_load(returns)
