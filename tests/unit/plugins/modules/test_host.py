...
"""Unit tests for scvmm_host module documentation stub."""
...
class TestArgumentSpec:
    ...
    def test_credential_option(self, parsed_doc):
        """The 'credential' option names a RunAs account, not a raw password."""
        opts = parsed_doc.get("options", {})
        assert "credential" in opts
        assert opts["credential"]["type"] == "str"

    def test_host_group_option(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        assert "host_group" in opts

    def test_reassociate_option(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        assert "reassociate" in opts
        assert opts["reassociate"]["type"] == "bool"
        assert opts["reassociate"]["default"] is False
...
class TestReturn:
    ...
    def test_return_has_host_key(self, module_docs):
        _, _, returns = module_docs
        parsed = yaml.safe_load(returns)
        ...
        for opt_name, opt_spec in opts.items():
            for pattern in SENSITIVE_PARAM_PATTERNS:
                if pattern in opt_name.lower():
                    # The "credential" field in scvmm_host refers to a
                    # RunAs account name, not a raw password.  Only fields
                    # containing "password" / "secret" / "token" need no_log.
                    if pattern == "credential" and "password" not in opt_name.lower():
                        continue
