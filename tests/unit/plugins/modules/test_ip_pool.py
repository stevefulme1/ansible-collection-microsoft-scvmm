...
"""Unit tests for scvmm_ip_pool module documentation stub."""
...
class TestArgumentSpec:
    ...
    def test_subnet_option(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        assert "subnet" in opts
        assert opts["subnet"]["type"] == "str"

    def test_ip_range_start_option(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        assert "ip_range_start" in opts

    def test_ip_range_end_option(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        assert "ip_range_end" in opts

    def test_gateway_option(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        assert "gateway" in opts
        assert opts["gateway"]["type"] == "list"
        assert opts["gateway"]["elements"] == "str"

    def test_dns_servers_option(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        assert "dns_servers" in opts
        assert opts["dns_servers"]["type"] == "list"
        assert opts["dns_servers"]["elements"] == "str"
...
class TestReturn:
    ...
    def test_return_has_ip_pool_key(self, module_docs):
        _doc, _examples, returns = module_docs
        parsed = yaml.safe_load(returns)
        assert "ip_pool" in parsed
