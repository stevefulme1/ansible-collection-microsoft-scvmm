...
"""Unit tests for scvmm_logical_network module documentation stub."""
...
class TestArgumentSpec:
    ...
    def test_network_virtualization_option(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        assert "network_virtualization_enabled" in opts
        assert opts["network_virtualization_enabled"]["type"] == "bool"
        assert opts["network_virtualization_enabled"]["default"] is False
...
class TestReturn:
    ...
    def test_return_has_logical_network_key(self, module_docs):
        _doc, _examples, returns = module_docs
        parsed = yaml.safe_load(returns)
        assert "logical_network" in parsed
