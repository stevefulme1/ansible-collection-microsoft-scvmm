...
"""Unit tests for scvmm_user_role module documentation stub."""
...
class TestArgumentSpec:
    ...
    def test_profile_option(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        ...
        assert set(opts["profile"]["choices"]) == expected

    def test_members_option(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        assert "members" in opts
        assert opts["members"]["type"] == "list"
        assert opts["members"]["elements"] == "str"
...
class TestReturn:
    ...
    def test_return_has_user_role_key(self, module_docs):
        _, _, returns = module_docs
        parsed = yaml.safe_load(returns)
        assert "user_role" in parsed
