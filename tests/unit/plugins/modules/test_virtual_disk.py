...
"""Unit tests for scvmm_virtual_hard_disk module documentation stub."""
...
class TestArgumentSpec:
    ...
    def test_size_gb_option(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        assert "size_gb" in opts
        assert opts["size_gb"]["type"] == "int"

    def test_disk_type_option(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        assert "disk_type" in opts
        assert set(opts["disk_type"]["choices"]) == {"Dynamic", "Fixed"}
        assert opts["disk_type"]["default"] == "Dynamic"

    def test_format_option(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        assert "format" in opts
        assert set(opts["format"]["choices"]) == {"VHD", "VHDX"}
        assert opts["format"]["default"] == "VHDX"

    def test_path_option(self, parsed_doc):
        opts = parsed_doc.get("options", {})
        assert "path" in opts
...
class TestReturn:
    ...
    def test_return_has_virtual_hard_disk_key(self, module_docs):
        _doc, _examples, returns = module_docs
        parsed = yaml.safe_load(returns)
        assert "virtual_hard_disk" in parsed
